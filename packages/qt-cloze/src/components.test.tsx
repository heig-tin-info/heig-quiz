...
describe("ClozePlayer", () => {
  ...
  it("reports a typed blank as a full array, one slot per blank", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith({ blanks: ["N", null, null, null, null] });
  });
  ...
  it("stores the canonical option index of a dropdown (decision D4)", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith({ blanks: [null, null, null, "2", null] });
  });
...
describe("ClozeReview", () => {
  ...
  it("shows a verdict per blank, not a bare score", () => {
    render(
      <ClozeReview
        student={student}
        answer={{ blanks: ["Newton", "9", null, "0", "n"] }}
  ...
  it("hides the expected column when the policy hides the key", () => {
    render(
      <ClozeReview
        student={student}
        answer={{ blanks: ["Newton", null, null, null, null] }}
        solution={null}
        details={details}
        points={2}
        maxPoints={6}
