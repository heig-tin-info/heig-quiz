...
/** The three types of WP2; `code` lands with WP3. */
const REGISTERED = ["mcq", "short", "cloze"] as const;

describe("the static registries", () => {
  it("hold the three types of WP2, in both halves", () => {
  ...
  it("reject an id that no package registers", () => {
    expect(() => questionType("code")).toThrow(UnknownQuestionType);
    expect(() => questionTypeClient("code")).toThrow(UnknownQuestionType);
