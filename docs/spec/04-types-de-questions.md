...
config:
  ...
  target: "0101…"        # the picture to draw, in the compact encoding below
```
...
- **The target** is captured by the teacher: in the editor, "Try the reference solution" runs it — in the browser for `runtime: runno`, else through `POST /questions/:id/try`, whose grading details carry the image — shows the picture it draws, and **"Use as target"** copies it into `config.target`. A canonical file may also write it by hand. A draft may lack it (decision D16); publication refuses a missing target (`codeimage.target_missing`), one of the wrong size (`codeimage.target_size`) or with a value outside the palette (`codeimage.target_value`).
