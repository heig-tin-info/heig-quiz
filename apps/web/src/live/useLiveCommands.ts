...
import { registerContextualCommands, type Command } from "../commands";
 ...
 * frame of every page. Registration is what "contextual" means.
 *
 * The registered callback is stable; it reads the current controls through a
 * ref, so a re-render of the dashboard does not churn the registry.
 */
  ...
  useEffect(
    () =>
      registerContextualCommands(() => {
        const { t, state, controls, navigate, id } = latest.current;
        const out: Command[] = [];
